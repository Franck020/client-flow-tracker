    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <DialogDescription>
                <StatusBadge hasSignal={client.hasSignal} monthsWithoutPayment={client.monthsWithoutPayment} />
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-3">
              {[
                ...
                { icon: MapPin, label: 'Localização', value: client.location },
                { icon: Wifi, label: 'TAP', value: client.tap },
                { icon: Calendar, label: 'Data do Contrato', value: format(client.contractDate, "dd 'de' MMMM 'de' yyyy", { locale: ptBR }) },
          ...
          {/* Financial Status */}
          <div className="space-y-4">
            <h3 className="font-semibold text-sm text-muted-foreground uppercase tracking-wider">Situação Financeira</h3>
            <div className={cn("p-4 rounded-lg", client.debt > 0 ? "bg-status-inactive-bg" : "bg-status-active-bg")}>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium">Dívida Atual</p>
                  <p className={cn("text-2xl font-bold", client.debt > 0 ? "text-status-inactive" : "text-status-active")}>
                ...
                {client.monthsWithoutPayment > 0 && (
                  <div className="text-right">
                    ...
                    <p className="text-lg font-bold text-status-warning">{client.monthsWithoutPayment}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* Signal Control */}
          <div className="space-y-4">
            <h3 className="font-semibold text-sm text-muted-foreground uppercase tracking-wider">Controlo do Sinal</h3>
            <div className="flex items-center justify-between p-4 rounded-lg bg-secondary/50">
              <div className="flex items-center gap-3">
                <SignalIndicator hasSignal={client.hasSignal} size="lg" />
                <div>
                  <p className="font-medium">{client.hasSignal ? 'Sinal Activo' : 'Sinal Cortado'}</p>
                  <p className="text-sm text-muted-foreground">{client.hasSignal ? 'Cliente a receber serviço' : 'Serviço suspenso'}</p>
                </div>
              </div>
              <Switch checked={client.hasSignal} onCheckedChange={() => onToggleSignal(client.id)} />
            </div>
          </div>
          ...
          {/* Payment */}
          <div className="space-y-4">
            <h3 className="font-semibold text-sm text-muted-foreground uppercase tracking-wider">Pagamento</h3>
            {!showPaymentForm ? (
              ...
              <div className="space-y-4 p-4 rounded-lg border bg-card">
                <div className="space-y-2">
                  ...
                  <Input id="amount" type="number" placeholder={`Sugerido: ${suggestedAmount.toLocaleString('pt-AO')} Kz`} value={paymentAmount} onChange={(e) => setPaymentAmount(e.target.value)} />
                  <div className="flex gap-2 flex-wrap">
                    <Button variant="outline" size="sm" onClick={() => setPaymentAmount(PAYMENT_CONFIG.MONTHLY_FEE.toString())}>
                      {PAYMENT_CONFIG.MONTHLY_FEE.toLocaleString('pt-AO')} Kz
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => setPaymentAmount((PAYMENT_CONFIG.MONTHLY_FEE + PAYMENT_CONFIG.LATE_FEE).toString())}>
                      {(PAYMENT_CONFIG.MONTHLY_FEE + PAYMENT_CONFIG.LATE_FEE).toLocaleString('pt-AO')} Kz (c/ multa)
                    </Button>
                    {client.debt > 0 && (
                      <Button variant="outline" size="sm" onClick={() => setPaymentAmount(client.debt.toString())}>
                        Quitar dívida
                      </Button>
                    )}
                  </div>
                ...
                <div className="flex gap-2">
                  <Button variant="outline" onClick={() => setShowPaymentForm(false)} className="flex-1">Cancelar</Button>
                  <Button onClick={handlePayment} disabled={!paymentAmount || parseFloat(paymentAmount) <= 0} className="flex-1">Confirmar</Button>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            {!showDeleteConfirm ? (
              ...
              >
                <Trash2 className="h-4 w-4 mr-2" /> Remover Cliente
              </Button>
            ) : (
