...
const Index = () => {
  ...
  const handleAddClient = (clientData: Parameters<typeof addClient>[0]) => {
        ...
        <div className="container flex h-16 items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Sistema de Gestão de Clientes</p>
            </div>
          </div>
      ...
      <main className="container py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard
            ...
            subtitle={`${activeClients.length} activos`}
            type="info"
          ...
          <StatCard
            ...
            subtitle="3+ meses sem sinal"
            type="warning"
            icon={<UserX className="h-5 w-5" />}
          />
          <StatCard
            title="Entradas Hoje"
            value={`${todayReport.totalEntradas.toLocaleString('pt-AO')} Kz`}
            subtitle={`${getTodayTransactions.filter(t => t.type === 'entrada').length} transacções`}
            type="income"
          />
          <StatCard
            title="Saídas Hoje"
            value={`${todayReport.totalSaidas.toLocaleString('pt-AO')} Kz`}
            subtitle={`${getTodayTransactions.filter(t => t.type === 'saida').length} transacções`}
            type="expense"
          />
        </div>
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div className="p-4 rounded-lg bg-income-bg">
                <p className="text-2xl font-bold text-income">
                  +{todayReport.totalEntradas.toLocaleString('pt-AO')} Kz
                </p>
                <p className="text-sm text-muted-foreground">Total Entradas</p>
              </div>
              <div className="p-4 rounded-lg bg-expense-bg">
                <p className="text-2xl font-bold text-expense">
                  -{todayReport.totalSaidas.toLocaleString('pt-AO')} Kz
                </p>
              ...
              <CardHeader>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <CardDescription>
                      {clients.length} clientes registados • Ordenados por código
                    </CardDescription>
                  </div>
              ...
              <CardContent>
                <ScrollArea className="h-[500px] pr-4">
                  <ClientList 
                    ...
                    emptyMessage={searchQuery ? "Nenhum cliente encontrado com esse termo" : "Nenhum cliente registado"}
                  />
                </ScrollArea>
              </CardContent>
            </Card>
          ...
          <TabsContent value="inactive" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Clientes com 3 ou mais meses sem sinal ou pagamento
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ScrollArea className="h-[500px] pr-4">
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      {getTodayTransactions.length} movimentações registadas
                    </CardDescription>
                  </div>
